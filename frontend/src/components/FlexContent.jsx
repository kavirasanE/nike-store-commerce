...
const FlexContent = ({ifExists, endpoint: { title,heading, text,img,url,btn}}) => {
  ...
  <div className= {`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse':'flex-row'} `}>
    ...
    <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full '>
        ...
        src={img} 
        alt={`img/${heading}`}
        className={`w-auto object-fill transition-theme ${ifExists ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 my-4 rotate-6 hover:-rotate-12':'h-72 lg:h-64 md:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`}/>
    </div>
  </div>
  </>
  )
