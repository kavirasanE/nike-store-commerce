      ...
      >
        <div className={`grid items-center ${ ifExists ? "justify-items-start" : "justify-items-center"}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" className=" bg-white/90 blur-effect-theme button-theme px-1 py-1  shadow shadow-sky-200 text-sm text-black">
        ...
        <div className={`flex items-center ${ifExists ? 'absolute top-5 right-1' :'justify-center' } `}>
          <img src={img} alt={`img/item/${id}`} className={`transition-theme hover:rotate-12 ${ifExists ? 'h-auto w-64 lg:w-56 md:w-48    -rotate-[35deg]': 'h-36 w-64'} `} />
        </div>
      </div>
    </>
  );
